...
def enter_option(buying_power, entries, trading_client, market_client, send_trade):
    ...
    for e in symbols:
        ...
        for o in options.option_contracts:
            ...
            dte = (datetime.now().date() - o.expiration_date).days
            if o.close_price != None and o.size != None and o.open_interest != None and dte > 0:
                size = float(o.size)
                ...
                is_within_bounds = abs(get_percentage_diff(shield, breakeven_price)) <= 10
                print("[ENTER] Option %s closed at %s with strike of %s and break even of %s" % (o.symbol, close_price, strike_price, breakeven_price))
